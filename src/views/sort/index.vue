...
const click_nav = async (index: number) => {
  try {
    ...
    if (Array.isArray(type_data.value)) {
      has_img.value = type_data.value.filter((item: any) => item.has_img === 0)
      null_img.value = type_data.value.filter((item: any) => item.has_img === 1)
    } else {
      has_img.value = null
      null_img.value = null
      return
  ...
  <div class="sort-page">
    ...
    >
      <template #content>
        ...
        <!-- 文字分类 -->
        <div class="text_sort" v-if="null_img != null">
          <div class="title">
            <a>枝数</a>
          </div>
          <van-row class="text_sort_style">
            <van-col span="8"><a>9枝</a> </van-col>
          </van-row>
        </div>
      </template>
    </van-tree-select>
